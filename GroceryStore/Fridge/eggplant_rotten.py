...
pygame.display.set_caption('Eggplant')
...
class Player(pygame.sprite.Sprite) :
    ...
    def update(self, keys, events) :
        ...
        if keys[pygame.K_w] and events[0] == 1 :
            self.rect.y -= self.init_speed
        elif keys[pygame.K_s] and events[0] == 1 :
            self.rect.y += self.init_speed

        if keys[pygame.K_a] and events[0] == 1 :
            self.rect.x -= self.init_speed
        elif keys[pygame.K_d] and events[0] == 1 :
            self.rect.x += self.init_speed
    ...
    for event in pygame.event.get() :
        ...
        if event.type == pygame.QUIT :
            ...
            if play_button_rect.collidepoint(pygame.mouse.get_pos()) :
                ...
                for i in range(1, HEIGHT_UNIT - 1) :
                    for j in range(1, WIDTH_UNIT - 1) :
                        pygame.draw.line(screen, GRAY, (i * WIDTH_UNIT, 0), (i * WIDTH_UNIT, WIDTH_UNIT * WIDTH_UNIT), 2)
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LEFT:
                        if not move_left:
                            player.update(-PATH_THICKNESS, 0)
                            move_left = True
                    elif event.key == pygame.K_RIGHT:
                        if not move_right:
                            player.update(PATH_THICKNESS, 0)
                        ...
                        if not move_up:
                            player.update(0, -PATH_THICKNESS)
                            move_up = True
                    elif event.key == pygame.K_DOWN:
                        if not move_down:
                            player.update(0, PATH_THICKNESS)
